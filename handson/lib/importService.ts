...
import { importApiRoot, pocApiRoot } from "./client";
...
export const importProducts = async (
  ...
  return importApiRoot
    ...
    .post({
      body: {
        resources: await getProductDraftImportArray(products.body.results),
        type: "product-draft",
      },
    })
    .execute();
...
const getProductDraftImportArray = async (
  products: Product[]
): Promise<Array<ProductDraftImport>> => {
  interface ProductData {
    [key: string]: any;
  }

  const extractPriceData = (price: {
    key: any;
    country: any;
    value: { currencyCode: any; centAmount: any; type: any };
  }) => {
    return {
      key: price.key,
  ...
  const productToProductDraftImport = (
    ...
    masterVariant: {
      ...
      prices:
        product.masterData.staged.masterVariant.prices.map(extractPriceData),
      key: product.masterData.staged.masterVariant.key,
      attributes: product.masterData.staged.masterVariant.attributes,
    },
    variants: product.masterData.staged.variants.map(
      (sourceVariant: {
        sku: any;
        prices: any;
        key: any;
